ANALYSIS SUMMARY
 
PROJECT GOAL
Identify why profit varies widely across states and measure how discounting impacts margins.
 
KEY METRICS ANALYZED
 Sales  
 Profit  
 Discount %  
 Loss due to discount  
 YoY (Year-over-Year) performance  
 
 
CORE INSIGHTS
1. Profitability by State
Mapping analysis revealed strong profit in several states, while others experienced negative margins despite high sales volume.
 
2. Discount Loss
High discounting directly correlated with negative profit.  
Total loss due to discount exceeded $154K across all states.
 
3. YoY Performance
Some states showed revenue growth, while others declined significantly, indicating inconsistent strategy execution.
 
 
SQL Contributions
Created cost column  
Computed profit metrics  
Isolated loss due to discount  
Added cost column for better understanding 
 
 
 
 
 
 
Power BI Deliverables
Profit heat map of all states  
Built YoY comparative tables
Loss-due-to-discount table 
Total Profit per State
KPI summary for executives  
Create Datetable for YoY comparison 
 
Final Outcome

This is a complete analytical solution showing:
Where profit is strongest  
Which states discount too aggressively  
How performance changes over time  
Actionable strategic recommendations

 
This project demonstrates my capabilities in:
Real-world BI workflow
SQL + Data Modeling
Business storytelling
Power BI dashboard design
Professional documentation

Contact If you’d like to connect or discuss:

Email: nyoresam@gmail.com
LinkedIn: https://www.linkedin.com/in/ememoh-oghenenyore-samuel5891311a6?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_
medium=android_app
Indeed: https://profile.indeed.com/p/oghenenyoresamuele-xx5sztm
PS.This project represents a complete data analyst workflow, built from scratch and optimized for a professional BI portfolio
 
